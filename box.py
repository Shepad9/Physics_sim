...
with open(f"configs_box.json/{SIM}.json", "r") as f:
    cfg = json.load(f)
...
def electric_field(pos, t, cfg):
    ...
    if E["TYPE"] == "cyclotron":
        if x > p["xmin"] and x < p["xmax"] and y > BOX[3]/2:
            return np.array([p["Ex"], p["Ey"]])
        if x > p["xmin"] and x < p["xmax"] and y <= BOX[3]/2:
            return np.array([-p["Ex"], -p["Ey"]])
        return np.zeros(2)
...
anim = FuncAnimation(
    fig, update, frames=6000, interval=4, blit=True
)

plt.show()
writer = FFMpegWriter(
    fps=1/STEP,
    metadata=dict(artist="Shepad"),
    bitrate=3000
)
print("saving")
